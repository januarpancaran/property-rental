    ...
    <div
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div
                ...
                <div
                    class="border-b border-indigo-100/50 bg-gradient-to-r from-indigo-50/80 via-white/70 to-indigo-50/50 px-6 py-4 dark:border-indigo-900/30 dark:from-indigo-950/30 dark:via-gray-900/50 dark:to-indigo-950/20">
                    <p class="text-xs uppercase tracking-wide font-bold text-indigo-600 dark:text-indigo-300">
                        {{ count($notifications) }} {{ count($notifications) === 1 ? 'Update' : 'Updates' }}</p>
                </div>
                <div class="space-y-0 divide-y divide-gray-100 dark:divide-gray-800/50 p-0">
                    @forelse ($notifications as $notification)
                        <div
                            class="group relative px-6 py-4 transition-all duration-200 hover:bg-indigo-50/30 dark:hover:bg-indigo-950/20">
                            <div class="flex items-start gap-4">
                                <div class="shrink-0 mt-1">
                                    ...
                                    @endphp
                                    <div
                                        class="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-{{ $iconColor }}-100 text-{{ $iconColor }}-700 dark:bg-{{ $iconColor }}-900/40 dark:text-{{ $iconColor }}-300">
                                        ...
                                        @elseif (str_contains($type, 'Maintenance'))
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 6V4m6 2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v10m6-6a2 2 0 100-4m0 4a2 2 0 110-4m0 4v10M9 6a2 2 0 100-4m0 4a2 2 0 110-4m0 4v10" />
                                            </svg>
                                        @else
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-2 flex items-center justify-between">
                                        <span
                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ $notification->created_at->diffForHumans() }}</span>
                    ...
                    @empty
                        <div class="px-6 py-12 text-center">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-500">You're all caught up! Check back later
                                for updates.</p>
                        </div>
                    @endforelse
                </div>
